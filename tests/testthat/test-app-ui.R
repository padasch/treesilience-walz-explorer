test_that("multi-run controls, metadata, plots, and protocols stay synchronized", {
  ...
  expect_false(grepl("qc-species", page_html, fixed = TRUE))
  expect_match(page_html, "qc-selection_mode", fixed = TRUE)
  expect_match(page_html, "qc-manual_runs", fixed = TRUE)
  expect_match(page_html, "qc-facet_columns", fixed = TRUE)
  expect_match(page_html, "Timeseries columns", fixed = TRUE)
  expect_match(page_html, "qc-normalize_a", fixed = TRUE)
  expect_match(page_html, "Normalize A by each run", fixed = TRUE)
  expect_match(page_html, 'id="qc-normalize_a"[^>]*checked', perl = TRUE)
  expect_match(page_html, "On by default so curves can be compared by shape", fixed = TRUE)
  expect_match(page_html, "qc-analyze_filtered", fixed = TRUE)
  expect_match(page_html, "Analyze filtered runs", fixed = TRUE)
  expect_match(page_html, "qc-analyze_all", fixed = TRUE)
  expect_match(page_html, "Analyze all runs", fixed = TRUE)
  expect_match(page_html, "No curves are prepared automatically", fixed = TRUE)
  expect_match(page_html, "qc-scope_notice", fixed = TRUE)
  expect_match(page_html, "A over elapsed time by quality assessment", fixed = TRUE)
  expect_match(page_html, "facet title identifies the run and its WALZ/XiBox settings", fixed = TRUE)
  expect_false(grepl("click a run-ID title", page_html, fixed = TRUE))
  expect_match(page_html, "Choose runs manually", fixed = TRUE)
  expect_match(page_html, "\"closeAfterSelect\":false", fixed = TRUE)
  expect_match(page_html, "Quality labels are read from the metadata sheet and are not editable in this app.", fixed = TRUE)
  expect_false(grepl("qc-review_controls", page_html, fixed = TRUE))
  expect_false(grepl("qc-set_good", page_html, fixed = TRUE))
  ...
  expect_match(page_html, "analysis-model_notice", fixed = TRUE)
  expect_match(page_html, "analysis-optima_section", fixed = TRUE)
